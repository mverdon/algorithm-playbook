...
import { dijkstra, dijkstraAnimated } from '@/algorithms/grid/dijkstra';
...
describe('Dijkstra Pathfinding Algorithm', () => {
  ...
  describe('dijkstraAnimated - Animation Steps', () => {
    it('should generate animation steps for simple path', () => {
      ...
      const grid = createGrid(config);
      const steps = Array.from(dijkstraAnimated(grid));
    ...
    it('should include distance in animation steps', () => {
      const config: GridConfig = {
        rows: 3,
        cols: 3,
        startPos: { row: 0, col: 0 },
    ...
    it('should not include heuristic in animation steps', () => {
      const config: GridConfig = {
        rows: 3,
        cols: 3,
        startPos: { row: 0, col: 0 },
      ...
      // Dijkstra doesn't use heuristic, so steps should not have heuristic field
      const visitingSteps = steps.filter(s => s.state === NodeState.Visiting);
      
      for (const step of visitingSteps) {
        expect(step.heuristic).toBeUndefined();
      }
    });
  ...
  describe('Dijkstra Algorithm Properties', () => {
    ...
    it('should visit more nodes than A* without heuristic', () => {
      const config: GridConfig = {
        rows: 10,
        cols: 10,
        startPos: { row: 0, col: 0 },
      ...
      expect(result.success).toBe(true);
      // Dijkstra explores nodes in all directions uniformly
      expect(result.visitedNodes.length).toBeGreaterThan(0);
      expect(result.visitedNodes.length).toBeLessThanOrEqual(100);
      // But it should still find the optimal path
    ...
    it('should handle multiple optimal paths consistently', () => {
      ...
      const result1 = dijkstra(grid);
      const result2 = dijkstra(grid);

      expect(result1.distance).toBe(result2.distance);
      expect(result1.success).toBe(result2.success);
    });

    it('should guarantee shortest path with uniform costs', () => {
      const config: GridConfig = {
        rows: 5,
        cols: 5,
        startPos: { row: 0, col: 0 },
      ...
      // Add some walls but ensure path exists
      grid[1][1].isWall = true;
      grid[1][1].state = NodeState.Wall;
      grid[2][3].isWall = true;
      grid[2][3].state = NodeState.Wall;

      const result = dijkstra(grid);

      expect(result.success).toBe(true);
      // Distance should be optimal even with obstacles
